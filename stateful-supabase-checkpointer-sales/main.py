...
def qualifier_agent(state: SalesState):
    ...
    Simple pattern: Ask question, then check response with keyword matching.
    """
    messages = state.get("messages", [])
    
    # If we haven't asked about budget yet, ask the question
    if state.get("can_afford") is None:
        response = AIMessage(content="Can you afford to invest $300 in this program?")
    ...
    # User has responded - check their answer with simple keyword matching
    last_message = messages[-1].content.lower() if messages else ""
    
    # Positive keywords
    positive = ["yes", "yeah", "yep", "sure", "afford", "can", "able", "definitely", "absolutely"]
    # Negative keywords
    negative = ["no", "nope", "can't", "cannot", "unable", "don't"]
    
    # Check for positive response
    if any(word in last_message for word in positive) and not any(word in last_message for word in negative):
        # User can afford - QUALIFY and route to closer
        response = AIMessage(
            content="Excellent! You're qualified for our program. Let me connect you with our closer who will help you complete your enrollment."
        )
        ...
        return {
            ...
            "can_afford": True,
            "budget": 300,
            "current_stage": "closing",  # This triggers routing to closer!
            "last_updated": datetime.now().isoformat()
        }
    
    # Check for negative response
    elif any(word in last_message for word in negative):
        # User cannot afford - DISQUALIFY
        response = AIMessage(
            content="I understand. Unfortunately, this program requires a $300 investment. Feel free to reach out when you're ready!"
        )
        ...
        response = AIMessage(
            content="I didn't quite catch that. Can you afford to invest $300 in this program? Please answer yes or no."
        )
        
        return {
            "messages": [response],
            "current_stage": "qualifying",  # Stay in qualifying
            "last_updated": datetime.now().isoformat()
        }
...
def route_conversation(state: SalesState) -> Literal["closer", "__end__"]:
    """
    Route to appropriate agent based on current stage.
    
    IMPORTANT: No self-loops! Each node processes once and ENDs.
    The next invoke() will start from the appropriate node based on current_stage.
    """
    current_stage = state.get("current_stage", "qualifying")
    
    # After qualifier runs, either go to closer or END
    if current_stage == "closing":
        return "closer"
    else:
        # For "qualifying", "disqualified", "complete" - END the graph
        # Next invoke will resume from the right place
        return END
...
def create_sales_graph():
    ...
    # Entry point - route based on current stage
    def route_entry(state: SalesState) -> Literal["qualifier", "closer"]:
    ...
    # After each node, END the graph (no self-loops!)
    # Conditional routing - NO SELF-LOOPS!
    workflow.add_conditional_edges(
        "qualifier",
        route_conversation,
        {
