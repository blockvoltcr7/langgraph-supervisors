"""
Stateful Workflows with Supabase Persistence Pattern (LangGraph v1)

This demonstrates how to build long-running workflows that:
1. Save state at every step (checkpointing to Supabase)
...
Updated for LangGraph v1 / LangChain v1 with Supabase:
- Using latest LangGraph v1 APIs
- Compatible with LangChain v1 ecosystem
- Production-ready persistence with PostgresSaver + Supabase
...
# Get DATABASE_URL and initialize checkpointer once at module level
_DATABASE_URL = os.getenv("DATABASE_URL")
if _DATABASE_URL:
    # Remove +psycopg from URL
    _db_url = _DATABASE_URL.replace('postgresql+psycopg://', 'postgresql://')
    
    # Create and setup checkpointer
    # We enter the context manager once and keep the connection open
    ...
    try:
        _CHECKPOINTER.setup()
        print(f"✅ Supabase checkpointer initialized")
    except Exception as e:
        print(f"⚠️  Warning: Could not setup checkpointer: {e}")
        _CHECKPOINTER = None
else:
    _CHECKPOINTER = None
    print("⚠️  No DATABASE_URL found - checkpointing disabled")
...
def create_project_workflow(database_url: str = None):
    """
    Create a stateful workflow with Supabase/PostgreSQL persistence.
    
    Args:
        database_url: PostgreSQL connection string (ignored - uses module-level checkpointer)
        
    Returns:
        Compiled graph with checkpointer
    """
    # Use the module-level checkpointer that was initialized at import time
    checkpointer = _CHECKPOINTER
    
    if not checkpointer:
        raise ValueError("Checkpointer not initialized. Make sure DATABASE_URL is set in .env")
...
# Note: This creates the graph once at module load time
# Tables must exist before this runs - use check_and_create_tables.py first
try:
    graph = create_project_workflow()
except Exception as e:
    print(f"⚠️  Warning: Could not create graph at module load: {e}")
    print("   Run 'python check_and_create_tables.py' first to create tables")
    graph = None
...
def main():
    """Demonstrate stateful workflow with Supabase persistence"""
    print("\n" + "="*80)
    print("STATEFUL WORKFLOW WITH SUPABASE PERSISTENCE DEMO")
    print("="*80 + "\n")
    
    # Create workflow with Supabase persistence
    graph = create_project_workflow()
    
    print(f"💾 Using Supabase database (PostgreSQL)")
    print(f"   (State persists in the cloud)\n")
